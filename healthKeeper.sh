...
update_dns() {
    ...
    if [ -z "$record_id" ]; then
        ...
        # 更新子域名的 IP 地址
        echo "更新子域名, domain=${subdomain}，当前IP=${current_ip}，更新为IP=${new_ip}"
        sleep 1
        local response=$(curl -s -X PUT "${api_url}/${record_id}"  \
            ...
            -H "Content-Type: application/json" \
            --data "{\"type\":\"A\",\"name\":\"${subdomain}\",\"content\":\"${new_ip}\",\"ttl\":1,\"proxied\":false}")
        echo "更新子域名, domain=${subdomain}，更新结果=${response}"
        local status=$(echo "$response" | jq -r '.success // ""')
        if [ "$status" = "true" ]; then
...
update_staticip() {
    ...
    echo "正在绑定新IP($new_ip) 到实例 ${instance_name}..."
    sleep 1
    ...
    if [[ $g_new_ip_addr =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
        ...
        echo "公网 IP 更新失败！实例: ${instance_name}"
...
EOF
    ...
    cron_command="*/3 * * * * /bin/bash /etc/v2ray-agent/healthKeeper.sh check >> /etc/v2ray-agent/logs/log_health_keeper.log 2>&1"
